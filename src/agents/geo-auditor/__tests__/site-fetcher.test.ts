...
import { assertPublicHost, fetchSite, MAX_HTML_BYTES, SiteFetchError } from '../site-fetcher.js';
...
describe('fetchSite', () => {
  ...
  it('descarga HTML y auxiliares en el origen final (tras redirect)', async () => {
    fetchSpy.mockImplementation(async (input: RequestInfo | URL) => {
      ...
      if (url === 'https://negocio.gt/') {
        return htmlResponse('<html><body>hola</body></html>', { url: 'https://www.negocio.gt/' });
      }
  ...
  it('lanza DNS en errores de red', async () => {
    ...
    await expect(fetchSite('https://no-existe-este-dominio.gt/')).rejects.toMatchObject({
      name: 'SiteFetchError',
      code: 'DNS',
    });
  });
