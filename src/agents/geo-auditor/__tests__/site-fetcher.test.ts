...
// El fetcher resuelve DNS para validar anti-SSRF; mockeamos node:dns/promises
// para no depender de la red y poder forzar la IP resuelta por host.
...
describe('fetchSite', () => {
  ...
  it('corta cadenas de redirect infinitas', async () => {
    fetchSpy.mockImplementation(async () => redirectTo('https://negocio.gt/loop'));
  ...
  it('lanza SiteFetchError con codigo HTTP_<status> en respuestas no-2xx', async () => {
    fetchSpy.mockResolvedValue(htmlResponse('not found', 404));
  ...
  it('no filtra el detalle interno en errores de conexion', async () => {
    fetchSpy.mockRejectedValue(new TypeError('connect ECONNREFUSED 10.1.2.3:80'));
    await expect(fetchSite('https://negocio.gt/')).rejects.toMatchObject({
      code: 'CONN',
  ...
  it('rechaza IP privada literal sin hacer fetch del body', async () => {
    fetchSpy.mockResolvedValue(htmlResponse('x'));
